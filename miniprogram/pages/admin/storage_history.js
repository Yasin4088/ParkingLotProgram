...
Page({
	...
	loadList: async function () {
		...
		try {
			let data = await cloudHelper.callCloudData('admin/storage_history_list', {
	...
	bindExportTap: async function () {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/report_storage_month', {
				yearMonth: this.data.yearMonth,
			}, { title: '生成报表中' });

			let urlData = await cloudHelper.callCloudData('admin/report_storage_url', {
				yearMonth: this.data.yearMonth,
			}, { title: '' });
			...
			fileHelper.openDoc('存取柜月报', urlData.url, '.xlsx');
		} catch (e) {
			console.log(e);
		} finally {
	...
	bindClearTap: function (e) {
		...
		wx.showModal({
			...
			success: async res => {
				...
				try {
					await cloudHelper.callCloudSumbit('admin/storage_history_clear', { id }, { title: '清理中' });
	...
	bindClearAllTap: function () {
		...
		wx.showModal({
			title: '清空历史记录',
			content: '将清理全部已取柜/已取消历史记录，此操作不可恢复，确认继续？',
			success: async res => {
				if (!res.confirm) return;
				try {
					await cloudHelper.callCloudSumbit('admin/storage_history_clear_all', {}, { title: '清理中' });
